...
VALUES   (@title,@content,@status,@add_time,@modify_time)";
        ...
        static void test_mult_insert()
        {
            ...
            {
                string sql_insert = @"
INSERT INTO [Content] (title, [content], status, add_time, modify_time)
 VALUES (@title,@content,@status,@add_time,@modify_time)";
...
WHERE   (id = @id)";
        ...
        static void test_select_one()
        {
            using (var conn = Db.OpenConnection())
            {
                ...
                var result = conn.QueryFirstOrDefault<Content>(sql_insert, new
                {
                    id = 5
                });
        ...
        static void test_select_list()
        {
            using (var conn = Db.OpenConnection())
            {
                ...
                var result = conn.Query<Content>(sql_insert, new
                {
                    ids = new int[] { 6, 7 }
                });
                Console.WriteLine($"test_select_one：查到的数据为：");
        ...
        static void test_select_content_with_comment()
        {
            using (var conn = new SqlConnection(""))
            {
                ...
                using (var result = conn.QueryMultiple(sql_insert, new
                {
                    id = 5
                }))
                {
